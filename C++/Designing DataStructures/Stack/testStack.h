...
public:
    void run()
    {
       ...
       test_pop_oneToEmpty();
       test_pop_standard();
       test_pop_standard_list();
       test_pop_standard_deque();
    ...
    // remove an element from a stack with one element
    void test_pop_oneToEmpty()
    {
       // setup
       custom::stack<Spy> s;
       s.container.push_back(Spy(99));
       Spy::reset();
       // exercise
       s.pop();
       // verify
       ...
       assertUnit(Spy::numAlloc() == 0);
       assertUnit(Spy::numAssign() == 0);
       assertUnit(Spy::numDelete() == 1);        // delete []
       assertUnit(Spy::numDefault() == 0);
       assertUnit(Spy::numNondefault() == 0);
       assertUnit(Spy::numCopyMove() == 0);
       assertUnit(Spy::numAssignMove() == 0);
       assertUnit(Spy::numDestructor() == 1);    // Calls Spy's destructor
       assertEmptyFixture(s);

       //teardown
    }

    // remove an element from a stack with many elements
    ...
    // remove an element from a stack (with std::list container) with many elements
    void test_pop_standard_list()
    {
       // setup
       //    +----+----+----+----+
       //    | 26 | 49 | 67 | 89 |
    ...
    // remove an element from a stack (with std::deque container) with many elements
    void test_pop_standard_deque()
    {
       // setup
       //    +----+----+----+----+
       //    | 26 | 49 | 67 | 89 |
       //    +----+----+----+----+
       custom::stack<Spy, std::deque<Spy>> s;
       s.container.push_back(Spy(26));
       s.container.push_back(Spy(49));
       s.container.push_back(Spy(67));
       s.container.push_back(Spy(89));
